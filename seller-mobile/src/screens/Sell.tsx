...
export function Sell() {
  ...
  // Sellers sell from the stock they hold.
  const available = units.filter((u) => u.status === 'with_seller' && u.heldBy === seller?.id && !used.has(u.serial));
  ...
  const generate = () => {
    const order = createOrder({ customerId, lines, discountTotal: billDisc, paidNow: Math.min(paying, grandTotal), method, soldBy: seller?.id ?? 'owner' });
    setPayOpen(false); setGenerated(order); toast('Invoice generated');
    setLines([]); setDiscVal(0); setPaidNow('');
  };

  return (
    <StackScreen title="New sale" sub="Scan or search your stock" noScroll noBack>
