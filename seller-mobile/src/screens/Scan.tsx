...
import { T, Btn, Input, Sheet, useToast } from '../ui';
import { Icon } from '../icons';
import { Money } from '../components';
import { getUnits, getProducts, categoryName, collectToSeller, returnToStorage } from '../data/db';
import { useAuth } from '../auth';

type Mode = 'collect' | 'sell' | 'return' | 'lookup';
const toggleModes: Mode[] = ['collect', 'sell', 'lookup'];
const label = (m: Mode) => m === 'collect' ? 'Collect' : m === 'sell' ? 'Sell' : m === 'return' ? 'Return' : 'Look up';
...
export function Scan() {
  ...
  const [mode, setMode] = useState<Mode>(route.params?.mode ?? 'collect');
  const [manual, setManual] = useState('');
  const [result, setResult] = useState<{ ok: boolean; serial: string; productId?: string; msg?: string } | null>(null);
  ...
  const resolve = (code: string) => {
    ...
    const prod = unit ? getProducts().find((p) => p.id === unit.productId) : getProducts().find((p) => p.barcode === s);

    if (mode === 'collect') {
      const ok = collectToSeller(s, seller?.id ?? '');
      toast(ok ? `Collected ${prod?.name ?? s}` : 'Not in storage / already taken', ok ? 'ok' : 'err');
      return;
    }
    if (mode === 'return') {
      const ok = returnToStorage(s);
      toast(ok ? 'Returned to storage' : 'Not held by you', ok ? 'ok' : 'err');
      return;
    }
    if (mode === 'sell') {
      if (unit && unit.status === 'with_seller' && unit.heldBy === seller?.id) { nav.navigate('Sell', { addSerial: unit.serial }); return; }
      setResult({ ok: false, serial: s, productId: unit?.productId ?? prod?.id, msg: 'Not in your stock — collect it first.' });
      return;
    }
  ...
  const isToggleMode = toggleModes.includes(mode);
    ...
    <View style={{ flex: 1, backgroundColor: '#04070D' }}>
      <View style={{ flex: 1 }}>
        ...
        <View style={{ position: 'absolute', top: insets.top + 8, left: 14, right: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {isToggleMode ? (
            <View style={{ flexDirection: 'row', backgroundColor: 'rgba(0,0,0,0.45)', borderRadius: 999, padding: 3 }}>
              {toggleModes.map((mm) => (
                <Pressable key={mm} onPress={() => setMode(mm)} style={{ paddingHorizontal: 14, height: 32, borderRadius: 999, alignItems: 'center', justifyContent: 'center', backgroundColor: mode === mm ? color.accent : 'transparent' }}>
                  <T w="s" size={12.5} c={mode === mm ? '#04140d' : '#E2E8F0'}>{label(mm)}</T></Pressable>
              ))}
            </View>
          ) : (
            <View style={{ backgroundColor: 'rgba(0,0,0,0.45)', borderRadius: 999, paddingHorizontal: 16, height: 32, justifyContent: 'center' }}>
              <T w="s" size={12.5} c="#E2E8F0">{label(mode)}</T></View>
          )}
          <View style={{ width: 40 }} />
        </View>
        ...
        <View pointerEvents="none" style={{ position: 'absolute', top: '56%', left: 0, right: 0, alignItems: 'center' }}>
          <View style={{ backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 999, paddingHorizontal: 14, paddingVertical: 7 }}>
            <T size={12.5} c="#E2E8F0" w="m">
              {mode === 'collect' ? 'Scan storage units to take them' : mode === 'sell' ? 'Scan your stock to add to a bill' : mode === 'return' ? 'Scan to return to storage' : 'Scan to look up a unit'}
            </T>
          </View>
        </View>
      </View>
      ...
      <Sheet open={!!result} onClose={() => setResult(null)} title={result?.ok ? 'Found' : 'Heads up'}>
        {result && (result.ok && product ? (
          ...
          <View style={{ alignItems: 'center', paddingVertical: 10, gap: 8 }}>
            ...
            <T w="s" size={15}>{result.msg ?? 'No match for'}</T>
