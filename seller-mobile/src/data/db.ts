...
// --- seller-specific ---
/** Collect an in-storage unit onto a seller (status -> with_seller, heldBy=seller). */
export function collectToSeller(serial: string, sellerId: string): boolean {
  const u = state.units.find((x) => x.serial.toLowerCase() === serial.toLowerCase());
  if (!u || u.status !== 'in_storage') return false;
  state = { ...state, units: state.units.map((x) => x.id === u.id ? { ...x, status: 'with_seller', heldBy: sellerId } : x) };
  emit();
  sync(() => api.saveUnit({ id: u.id, status: 'with_seller', heldBy: sellerId }));
  return true;
}
/** Return a unit held by a seller back to shop storage. */
export function returnToStorage(serial: string): boolean {
  const u = state.units.find((x) => x.serial.toLowerCase() === serial.toLowerCase());
  if (!u || u.status !== 'with_seller') return false;
  state = { ...state, units: state.units.map((x) => x.id === u.id ? { ...x, status: 'in_storage', heldBy: undefined } : x) };
  emit();
  sync(() => api.saveUnit({ id: u.id, status: 'in_storage', heldBy: undefined }));
  return true;
}
/** Mock seller login — match email + password against the sellers list. */
export function sellerLogin(email: string, password: string): Seller | null {
  const e = email.trim().toLowerCase();
  return state.sellers.find((s) => s.active && s.email?.toLowerCase() === e && s.password === password) ?? null;
}
export const getSeller = (id: string) => state.sellers.find((s) => s.id === id) ?? null;
